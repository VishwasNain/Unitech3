...
// Helper to chunk an array into arrays of n
...
const FancyFilterCard = ({ children }) => (
  <Paper
    elevation={8}
    sx={{
      p: 3,
      ...
      boxShadow: '0 8px 36px 0 #1a237e44',
      display: 'flex',
      flexWrap: 'wrap',
      gap: 2,
      alignItems: 'center',
      ...
      '&:hover': {
        ...
        boxShadow: '0 16px 48px 0 #00e5ff55',
        borderColor: '#00e5ff',
      },
...
const Products = () => {
  const [zoomedImgOpen, setZoomedImgOpen] = useState(false);
  ...
  const handleZoomOpen = (imagesArr, idx) => {
    setZoomImages(imagesArr);
    setZoomImgIdx(idx);
    setZoomOpen(true);
  };
  const handleZoomClose = () => setZoomOpen(false);
  const handleZoomPrev = () => setZoomImgIdx(idx => (idx - 1 + zoomImages.length) % zoomImages.length);
  const handleZoomNext = () => setZoomImgIdx(idx => (idx + 1) % zoomImages.length);

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'auto' });
  }, [location.pathname]);

  // Initialize products data with all your original products
  const productsByCategory = {
    laptops: [
      {
        id: "64f9c0ab1f5b2b7d12345670",
  ...
  // --- LAPTOP FILTER STATE ---
  const [laptopBrand, setLaptopBrand] = useState('all');
  const [laptopProcessor, setLaptopProcessor] = useState('all');
  const [laptopRam, setLaptopRam] = useState('all');
  const [laptopStorage, setLaptopStorage] = useState('all');
  const [laptopPrice, setLaptopPrice] = useState([0, 100000]);
  const [sliderValue, setSliderValue] = useState([0, 100000]);
  const debounceRef = useRef();

  // --- DYNAMIC FILTER OPTIONS ---
  const laptops = productsByCategory.laptops || [];
  const brands = Array.from(new Set(laptops.map(l => l.brand)));
  const processors = Array.from(new Set(laptops.map(l => l.specs.processor)));
  const rams = Array.from(new Set(laptops.map(l => l.specs.ram)));
  const storages = Array.from(new Set(laptops.map(l => l.specs.storage)));
  const minPrice = Math.min(...laptops.map(l => l.price));
  const maxPrice = Math.max(...laptops.map(l => l.price));

  useEffect(() => {
    setLaptopPrice([minPrice, maxPrice]);
  ...
  const handleSliderChange = (e, newVal) => {
    ...
    debounceRef.current = setTimeout(() => {
      setLaptopPrice(newVal);
    }, 300);
  };

  // --- MEMOIZED FILTERED LAPTOPS ---
  const filteredLaptops = useMemo(() => laptops.filter(l => {
    return (
      (laptopBrand === 'all' || l.brand === laptopBrand) &&
      (laptopProcessor === 'all' || l.specs.processor === laptopProcessor) &&
      (laptopRam === 'all' || l.specs.ram === laptopRam) &&
      (laptopStorage === 'all' || l.specs.storage === laptopStorage) &&
      (l.price >= laptopPrice[0] && l.price <= laptopPrice[1])
    );
  }), [laptops, laptopBrand, laptopProcessor, laptopRam, laptopStorage, laptopPrice]);
    ...
    <FancyFilterCard>
      <Box display="flex" alignItems="center" gap={1}>
        ...
        <FormControl sx={{ minWidth: 120 }} size="small">
          <InputLabel sx={{ color: '#e3f6fd' }}>Brand</InputLabel>
          <Select value={laptopBrand} label="Brand" onChange={e => setLaptopBrand(e.target.value)}>
      ...
      <Box display="flex" alignItems="center" gap={1}>
        <LaptopMacIcon sx={{ color: '#00e5ff' }} />
        <FormControl sx={{ minWidth: 140 }} size="small">
          <InputLabel sx={{ color: '#e3f6fd' }}>Processor</InputLabel>
          <Select value={laptopProcessor} label="Processor" onChange={e => setLaptopProcessor(e.target.value)}>
            <MenuItem value="all">All</MenuItem>
            {processors.map(p => <MenuItem key={p} value={p}>{p}</MenuItem>)}
          </Select>
        </FormControl>
      </Box>
      <Box display="flex" alignItems="center" gap={1}>
        <MemoryIcon sx={{ color: '#00e5ff' }} />
        <FormControl sx={{ minWidth: 120 }} size="small">
          <InputLabel sx={{ color: '#e3f6fd' }}>RAM</InputLabel>
          <Select value={laptopRam} label="RAM" onChange={e => setLaptopRam(e.target.value)}>
            <MenuItem value="all">All</MenuItem>
      ...
      <Box display="flex" alignItems="center" gap={1}>
        <SdStorageIcon sx={{ color: '#00e5ff' }} />
        <FormControl sx={{ minWidth: 140 }} size="small">
          <InputLabel sx={{ color: '#e3f6fd' }}>Storage</InputLabel>
          <Select value={laptopStorage} label="Storage" onChange={e => setLaptopStorage(e.target.value)}>
            <MenuItem value="all">All</MenuItem>
            {storages.map(s => <MenuItem key={s} value={s}>{s}</MenuItem>)}
          </Select>
        </FormControl>
      </Box>
      <Divider orientation="vertical" flexItem sx={{ mx: 2, display: { xs: 'none', sm: 'block' } }} />
      <Box display="flex" flexDirection="column" alignItems="flex-start" sx={{ minWidth: 220, px: 2 }}>
        <Box display="flex" alignItems="center" gap={1}>
          <AttachMoneyIcon sx={{ color: '#00e5ff' }} />
          <Typography variant="body2" sx={{ color: '#e3f6fd', fontWeight: 600 }}>Price Range</Typography>
        </Box>
        <Slider
          value={sliderValue}
          min={minPrice}
      ...
      <Button
        ...
        sx={{
          ...
          fontWeight: 700,
          px: 3,
          background: 'linear-gradient(90deg,#00e5ff 30%,#7f53ff 100%)',
          color: '#fff',
          boxShadow: '0 2px 8px #2563eb22',
          textTransform: 'none',
          transition: 'background 0.18s, box-shadow 0.18s',
          '&:hover': {
            background: 'linear-gradient(90deg,#2563eb 10%,#06b6d4 90%)',
            boxShadow: '0 4px 16px #2563eb33',
          },
        }}
        onClick={() => {
          setLaptopBrand('all');
          setLaptopProcessor('all');
          setLaptopRam('all');
          setLaptopStorage('all');
          setLaptopPrice([minPrice, maxPrice]);
          setSliderValue([minPrice, maxPrice]);
        }}
      >
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#23272f' }}>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <FormControl size="small" sx={{ minWidth: 120, mb: 3, mb: 3 }}>
          <InputLabel sx={{ color: '#e3f6fd' }}>Condition</InputLabel>
          <Select
        ...
        {category === 'laptops' && renderLaptopFilters()}
        {category === 'laptops' ? (
          <>
            {/* Mobile: 3 per row, Desktop: Grid */}
            <Box sx={{ display: { xs: 'block', md: 'none' } }}>
              {chunkArray(
                conditionFilteredProducts.filter(product => selectedBrand === 'all' || product.brand === selectedBrand),
                3
              ).map((row, rowIdx) => (
                <Box
                  key={rowIdx}
                ...
                >
                  {row.map(product => (
                    ...
                    >
                      <StyledCard sx={{
                        ...
                        {(product.category === 'accessories' || product.Category === 'accessories') && (
                          <Box sx={{ position: 'absolute', top: 12, left: 12, zIndex: 1 }}>
                            <Chip label="3 Year Warranty" color="success" size="small" sx={{ fontWeight: 700, bgcolor: '#22c55e', color: '#fff', boxShadow: 1, borderRadius: 1 }} />
                          </Box>
                        )}
                        <Box onClick={() => handleZoomOpen(product.images || [product.image], 0)} sx={{ cursor: 'zoom-in' }}>
                          <StyledProductImage
                            component="img"
                        ...
                        <StyledCardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 180 }}>
                          <div>
                  ...
                  {row.length < 3 && Array.from({ length: 3 - row.length }).map((_, i) => (
                    <Box key={i} sx={{ flex: '0 0 80vw', minWidth: 260, maxWidth: 300, minHeight: 1 }} />
                  ))}
                </Box>
              ))}
            </Box>
  ...
  >
    ...
    />
        ...
        {/* Image Zoom Modal */}
        <Dialog open={zoomedImgOpen} onClose={() => setZoomedImgOpen(false)} maxWidth="md" PaperProps={{ style: { background: 'rgba(20,24,32,0.82)', boxShadow: 'none', backdropFilter: 'blur(4px)' } }}>
          <Fade in={zoomedImgOpen} timeout={320}>
            <Box
              sx={{
                display: 'flex',
                alignItems: 'center',
              ...
              onClick={() => setZoomedImgOpen(false)}
            >
              {zoomedImg && (
                <img
                  src={zoomedImg}
              ...
              <IconButton
                aria-label="close"
                onClick={() => setZoomedImgOpen(false)}
                sx={{ position: 'absolute', top: 12, right: 12, color: '#fff', zIndex: 2, background: 'rgba(30,30,30,0.4)', backdropFilter: 'blur(2px)' }}
              >
                <CloseIcon />
              </IconButton>
            </Box>
